...
const nav = [
  ...
  { href: '/suscripcion', label: 'Suscripcion', icon: 'M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3M3.75 18.75h16.5a1.5 1.5 0 001.5-1.5V6.75a1.5 1.5 0 00-1.5-1.5H3.75a1.5 1.5 0 00-1.5 1.5v10.5a1.5 1.5 0 001.5 1.5z' },
  ];
...
export default function Sidebar() {
  ...
  useEffect(() => {
    getMyBusiness().then((b) => { if (b) setBizName(b.name); });
  }, []);
  return (
  ...
  <p className="text-lg font-semibold mt-0.5">{bizName || 'Mi negocio'}</p>
  </div>
<nav className="flex-1 px-3 py-4 space-y-1">
{nav.map((item) => {
  const active = pathname === item.href;
  return (
    <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${active ? 'bg-brand text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}>
